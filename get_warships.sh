...
get_page() {
	curl -s "https://api.worldofwarships.eu/wows/encyclopedia/ships/?application_id=80aa6f7e1d5df049c79d9141dd0826ae&fields=name,price_gold,nation,is_premium,ship_id,price_credit,tier,next_ships&limit=100&page_no=${1}" | jq  '.data | to_entries | map(.value)'
}
